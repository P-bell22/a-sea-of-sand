import { MAX_SIMULATION_SPEED } from './wind';
...
export function validateSettings(input:unknown):ToolInput{
 ...
 for(const [key,min,max] of [['speed',1,MAX_SIMULATION_SPEED],['strength',0,2],['altitude',90,700],['direction',0,360]] as const){if(key in s&&(typeof s[key]!=='number'||!Number.isFinite(s[key])||Number(s[key])<min||Number(s[key])>max))throw new Error(`${key} must be between ${min} and ${max}`);}
...
export function registerModelTools(read:()=>SimState,apply:(input:ToolInput)=>void){
 ...
 const tool:Tool={name:'configure_dune_simulation',description:'Set dune simulation playback, viewpoint, time speed, sand transport, wind direction, automatic wind variation or altitude. Reset restarts the elapsed time and camera.',inputSchema:{type:'object',properties:{playing:{type:'boolean'},view:{type:'string',enum:['aerial','above','grain']},speed:{type:'number',minimum:1,maximum:MAX_SIMULATION_SPEED},strength:{type:'number',minimum:0,maximum:2},altitude:{type:'number',minimum:90,maximum:700},reset:{type:'boolean'},direction:{type:'number',minimum:0,maximum:360},variableWind:{type:'boolean'}},additionalProperties:false},annotations:{readOnlyHint:false,untrustedContentHint:false},execute:async(input)=>{apply(validateSettings(input));await new Promise(requestAnimationFrame);const {model,...settings}=read();return {...settings,field:model?.stats()??null};}};
