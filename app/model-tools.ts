import type { SimState } from './terrain';
type ToolInput={playing?:boolean;view?:string;speed?:number;strength?:number;altitude?:number;reset?:boolean};
export function validateSettings(input:unknown):ToolInput{
 ...
 for(const key of Object.keys(s))if(!['playing','view','speed','strength','altitude','reset'].includes(key))throw new Error(`Unknown setting: ${key}`);
 for(const key of ['playing','reset'])if(key in s&&typeof s[key]!=='boolean')throw new Error(`${key} must be boolean`);
 if('view' in s&&!['aerial','above','grain'].includes(s.view as string))throw new Error('Invalid viewpoint');
 for(const [key,min,max] of [['speed',1,50],['strength',0,2],['altitude',160,700]] as const){if(key in s&&(typeof s[key]!=='number'||!Number.isFinite(s[key])||Number(s[key])<min||Number(s[key])>max))throw new Error(`${key} must be between ${min} and ${max}`);}
