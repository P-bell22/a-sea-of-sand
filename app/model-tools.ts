...
export function validateSettings(input:unknown):ToolInput{
 ...
 for(const [key,min,max] of [['speed',1,50],['strength',0,2],['altitude',90,700],['direction',0,360]] as const){if(key in s&&(typeof s[key]!=='number'||!Number.isFinite(s[key])||Number(s[key])<min||Number(s[key])>max))throw new Error(`${key} must be between ${min} and ${max}`);}
